...
function CheckInPortalPageContent() {
    ...
    <div className="max-w-5xl mx-auto space-y-8">
      <div className="border-b pb-4">
        <h1 className="text-3xl font-extrabold tracking-tight text-neutral-900 dark:text-neutral-50 flex items-center gap-2">
          ...
          <span>On-Site Check-In Scanner Portal</span>
        </h1>
        <p className="mt-1 text-sm text-neutral-500">
          Select an active or upcoming event to launch the QR scanner, look up participant bib numbers, and process race pack collections (REPC).
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <EmptyState
          icon={QrCode}
          title="No Events Ready for Check-In"
          description="An event manager must create and publish an event before race-pack check-in can begin."
          action={{
            label: "Create Event",
              ...
              <CardContent className="pt-2 space-y-4">
                <div className="bg-neutral-50 dark:bg-neutral-800/60 p-3 rounded-xl flex items-center justify-between text-xs text-neutral-600 dark:text-neutral-300 border border-neutral-100 dark:border-neutral-800">
                  <span className="flex items-center gap-1.5 font-semibold">
                    <Users className="h-4 w-4 text-primary-500" /> Total Registrations
                  </span>
                ...
                <div className="pt-2 border-t flex justify-end">
                  ...
                  >
                    <Link href={`/dashboard/events/${ev.id}/operations/checkin`}>
                      Launch QR Scanner <ArrowRight className="h-4 w-4" />
                    </Link>
                  </Button>
                </div>
              </CardContent>
