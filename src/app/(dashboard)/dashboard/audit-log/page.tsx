...
function AuditLogPageContent() {
  ...
  const { data, isLoading, error } = trpc.settings.getAuditLogs.useQuery({
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <div>
        <p className="text-xs font-bold uppercase tracking-[0.18em] text-primary-600">Platform oversight</p>
        <h1 className="mt-1 flex items-center gap-2 text-3xl font-black tracking-tight text-neutral-900 dark:text-neutral-50">
          <ClipboardList className="h-7 w-7 text-primary-500" /> Audit log
        </h1>
        <p className="mt-2 text-sm text-neutral-500">A read-only history of approvals, payments, settings, and operational actions.</p>
      </div>
      ...
      {isLoading ? (
        <Card className="overflow-hidden rounded-2xl border-neutral-200 dark:border-neutral-800" aria-label="Loading...">
          <div role="region" aria-label="Loading audit log" tabIndex={0} className="dashboard-scroll-region">
        ...
        <EmptyState
          ...
          description="Try adjusting your search terms or clearing the filter to see all audit logs."
        />
      ) : (
        <Card className="overflow-hidden rounded-2xl border-neutral-200 dark:border-neutral-800">
          <div role="region" aria-label="Audit log, scroll horizontally for all columns" tabIndex={0} className="dashboard-scroll-region outline-none focus-visible:ring-2 focus-visible:ring-primary-500">
            <table className="min-w-[44rem] w-full text-left text-sm">
              <thead className="bg-neutral-50 text-[11px] font-bold uppercase tracking-wide text-neutral-500 dark:bg-neutral-900/80">
                <tr>
                  <th scope="col" className="px-5 py-3">When</th>
